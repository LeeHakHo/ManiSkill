...
def convert_obs(obs, concat_fn, transpose_fn, state_obs_extractor, depth=True):
    ...
    if (
        "depth" in new_img_dict
        and isinstance(new_img_dict["depth"], torch.Tensor)
    ...
    for x in states_to_stack:
        ...
        # 차원 맞추기: 최종적으로 (T, D) 꼴로 맞춰서 hstack 가능하게
        if arr.ndim == 0:
            # 스칼라 → (1, 1)
            arr = arr.reshape(1, 1)
        elif arr.ndim == 1:
    ...
    try:
        ...
        # 혹시 모를 예외 대비 fallback
        state = np.column_stack(processed_states)

    if state.dtype == np.float64:
        for x in processed_states:
